...
func openTestCorpus(t *testing.T) (*Corpus, string) {
	...
	testCorpusTemplateOnce.Do(func() {
		...
		c, err := Open(ctx, testCorpusTemplatePath)
		if err != nil {
			errTestCorpusTemplate = err
			return
		}
	...
	if err := copyTestDatabase(testCorpusTemplatePath, path); err != nil {
...
func TestTestCorpusTemplateIsCurrentAndStandalone(t *testing.T) {
	...
	if _, err := os.Stat(testCorpusTemplatePath + "-wal"); !errors.Is(err, os.ErrNotExist) {
