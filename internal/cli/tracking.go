...
func (c *CLI) runTrackingExport(ctx context.Context, cmd *trackingExportCmd, service TrackingService) error {
	...
	if cmd.Output != "" {
		...
		if _, err := fmt.Fprintf(c.stderr, "wrote tracking metadata export v%d to %s (%d triage, %d contributions, %d outcomes, %d evidence)\n",
			result.SchemaVersion, cmd.Output, result.TriageEvents, result.Contributions, result.ContributionOutcomes, result.Evidence); err != nil {
			return c.mapError(err)
		}
		return nil
	}
