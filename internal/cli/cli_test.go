...
type fakeService struct {
	...
	lastRadarOpts      cli.RadarOptions
...
func (f *fakeService) ContributionRadar(_ context.Context, opts cli.RadarOptions) (*radar.Report, error) {
	f.radarCalled = true
	f.lastRadarOpts = opts
	return f.radarResult, f.err
}
...
func TestRadarRendersExplanations(t *testing.T) {
	...
	svc := &fakeService{radarResult: &radar.Report{
	...
	if !svc.radarCalled || svc.lastRadarOpts.Repo.String() != "o/r" || svc.lastRadarOpts.Limit != 20 {
		t.Fatalf("radar call = called:%v opts:%+v", svc.radarCalled, svc.lastRadarOpts)
	}
...
func TestRadarJSONAndLimitValidation(t *testing.T) {
	svc := &fakeService{radarResult: &radar.Report{Repo: "o/r", ScoreVersion: radar.ScoreVersion, Candidates: []radar.Candidate{}}}
	c, stdout, _ := newTestCLI(svc, nil)
	...
	if got.Repo != "o/r" || svc.lastRadarOpts.Limit != 5 {
		t.Fatalf("result=%+v opts=%+v", got, svc.lastRadarOpts)
	}

	svc.radarCalled = false
