...
func (s *Server) registerConcernTools() {
	...
	addCatalogTool(s, catalogTool[mcpcontract.ListConcernsInput, mcpcontract.ConcernListOutput]{
		...
		description: "List or search compact triage summaries from the repo-local concern ledger. Truncated results return a typed next-offset action. Read a returned resource URI for the complete concern. This never contacts GitHub, reads a worktree, or executes code.",
