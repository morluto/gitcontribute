...
func (r *MCPReader) SearchGitHubRepositories(ctx context.Context, in mcpserver.SearchGitHubRepositoriesInput) (mcpserver.SearchGitHubRepositoriesOutput, error) {
	...
	if in.Limit < 1 || in.Limit > 100 {
		return mcpserver.SearchGitHubRepositoriesOutput{}, mcpserver.InvalidArgument("limit", "must be between 1 and 100", map[string]any{"limit": 20})
	}
	if in.Sort != "" && in.Sort != "stars" && in.Sort != "forks" && in.Sort != "help-wanted-issues" && in.Sort != "updated" {
		return mcpserver.SearchGitHubRepositoriesOutput{}, mcpserver.InvalidArgument("sort", "must be stars, forks, help-wanted-issues, or updated", map[string]any{"sort": "stars"})
	}
	if in.Order != "" && in.Order != "asc" && in.Order != "desc" {
		return mcpserver.SearchGitHubRepositoriesOutput{}, mcpserver.InvalidArgument("order", "must be asc or desc", map[string]any{"order": "desc"})
	}
	if in.Page == 0 {
		in.Page = 1
	}
	if in.Page < 1 || in.Page > 1000 || (in.Page-1)*in.Limit >= 1000 {
		return mcpserver.SearchGitHubRepositoriesOutput{}, mcpserver.InvalidArgument("page", "must keep the requested result offset below GitHub's 1,000-result cap", map[string]any{"page": 1, "limit": in.Limit})
	}
	if in.ResponseFormat == "" {
		in.ResponseFormat = "detailed"
	}
	if in.ResponseFormat != "concise" && in.ResponseFormat != "detailed" {
		return mcpserver.SearchGitHubRepositoriesOutput{}, mcpserver.InvalidArgument("response_format", "must be concise or detailed; use concise for discovery and detailed for finalist inspection", map[string]any{"response_format": "concise"})
	}
	...
	for i, remote := range result.Items {
		...
		item := mcpserver.BatchItem[mcpserver.RepositorySearchMatch]{Key: key, Status: "complete"}
	...
	if len(result.Items) > 0 {
		...
		if len(actionItems) > 50 {
			actionItems = actionItems[:50]
		...
		repositories := make([]mcpserver.RepositoryRef, 0, len(actionItems))
		for _, remote := range actionItems {
...
func compileRepositorySearch(in mcpserver.SearchGitHubRepositoriesInput) (string, string, []mcpserver.SearchWarning, error) {
	...
	if legacy != "" && raw != "" {
		return "", "", nil, mcpserver.InvalidArgument("raw_query", "query and raw_query are mutually exclusive; use raw_query for advanced provider syntax", map[string]any{"raw_query": "topic:cuda stars:>500"})
	}
	if (legacy != "" || raw != "") && structured {
		return "", "", nil, mcpserver.InvalidArgument("raw_query", "cannot be combined with structured filters; choose one input mode", map[string]any{"text": "inference", "topics": []string{"cuda"}})
	}
	if legacy == "" && raw == "" && !structured {
		return "", "", nil, mcpserver.InvalidArgument("text", "provide raw_query or at least one structured filter such as text, topics, language, or pushed_after", map[string]any{"text": "inference", "match_fields": []string{"name", "description"}})
	}
	if len(in.MatchFields) > 0 && strings.TrimSpace(in.Text) == "" {
		return "", "", nil, mcpserver.InvalidArgument("match_fields", "requires text", map[string]any{"text": "inference", "match_fields": []string{"name", "description"}})
	}
	for _, field := range in.MatchFields {
		if field != "name" && field != "description" && field != "readme" {
			return "", "", nil, mcpserver.InvalidArgument("match_fields", "values must be name, description, or readme", map[string]any{"match_fields": []string{"name", "description"}})
		}
	}
	query := raw
	...
	if legacy != "" {
		query = legacy
		mode = "legacy raw query"
		warnings = append(warnings, mcpserver.SearchWarning{Code: "deprecated_query", Message: "query is retained for compatibility but exposes provider-specific syntax.", Suggestion: "Use structured filters or raw_query."})
	} else if raw != "" {
		mode = "advanced raw query"
	}
	if query != "" {
		if strings.Contains(strings.ToLower(query), "in:readme") {
			warnings = append(warnings, mcpserver.SearchWarning{Code: "broad_readme_match", Message: "README matching can include incidental mentions and unrelated repositories.", Suggestion: "Prefer name, description, topic, or language filters for discovery."})
		}
	...
	if len(in.MatchFields) > 0 {
		parts = append(parts, "in:"+strings.Join(in.MatchFields, ","))
		for _, field := range in.MatchFields {
			if strings.EqualFold(strings.TrimSpace(field), "readme") {
				warnings = append(warnings, mcpserver.SearchWarning{Code: "broad_readme_match", Message: "README matching can include incidental mentions and unrelated repositories.", Suggestion: "Prefer name, description, topic, or language filters for discovery."})
				break
			}
		}
	...
	return strings.Join(parts, " "), "Search using structured repository filters.", warnings, nil
}
